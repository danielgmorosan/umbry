...
| F4 | **Uploads: unauthenticated POST, no quota/rate limit.** Per-file cap exists (`MAX_UPLOAD_BYTES`), but nothing stops disk-filling loops. GET is capability-URL (unguessable UUID) — acceptable, but no membership binding. | **High** | Abuse/DoS vector on Mode A; storage cost risk. **✅ Fixed (live):** `RELAY_REQUIRE_AUTH` requires a token + records the uploader; **per-IP HTTP rate limit (300/10s) and per-socket WS message cap (~30/s) now live.** Per-user storage *quota* still deferred to D5. |
...
| F7 | **`CORS_ORIGIN` defaults to `*`**; no security headers on upload/file responses beyond `nosniff`. | Medium | Fine for dev; must be pinned in production. **✅ Fixed (live):** `CORS_ORIGIN` set to the prod origin — HTTP CORS echoes an allowlisted origin and the WS upgrade is rejected (`verifyClient`) for any other origin. Verified: real origin connects, bogus origin 401. |
...
|---|---|---|---|
| **D1. Desktop shell** | Electron app running the existing renderer | ✅ **First increment shipped (2026-07-16):** `apps/desktop` Electron shell — hardened BrowserWindow (contextIsolation/sandbox on, nodeIntegration off, origin-pinned navigation, external links to system browser), preload exposes only `window.gossipDesktop`. Loads the deployed app; verified via Playwright-Electron (hardening 6/6 + real-app-against-locked-relay 4/4). **⏳ Remaining:** app icon/branding, native-notification wiring + click-to-focus, `electron-builder` + GitHub Actions matrix (mac/linux/win) for signed/notarized installers, auto-update. | ~1 week |
